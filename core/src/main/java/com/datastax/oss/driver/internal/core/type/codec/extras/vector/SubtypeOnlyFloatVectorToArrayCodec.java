...
import com.datastax.oss.driver.internal.core.type.SubtypeOnlyVectorType;
...
/**
 * Implementation of {@link TypeCodec} which translates CQL vectors into float arrays.
 * Difference between this and {@link FloatVectorToArrayCodec} is that we don't concern
 * ourselves with the dimensionality specified in the input CQL type.  This codec just
 * reads all the bytes, tries to deserislize them consecutively into subtypes and then
 * returns the result.  Serialiation is similar: we take the input array, serialize
 * each element and return the result.
 */
public class SubtypeOnlyFloatVectorToArrayCodec implements TypeCodec<float[]> {
    ...
    // TODO: Not sure why we didn't do this for FloatVectorToArrayCodec...
    private final FloatCodec floatCodec = new FloatCodec();

    public SubtypeOnlyFloatVectorToArrayCodec(
            @NonNull DataType subType) {
        this.cqlType = new SubtypeOnlyVectorType(Objects.requireNonNull(subType, "subType cannot be null"));
        this.javaType = GenericType.of(float[].class);
    }
    ...
    public float[] decode(@Nullable ByteBuffer bytes, @NonNull ProtocolVersion protocolVersion) {
        ...
        // TODO: Do we want to treat this as an error?  We could also just ignore any extraneous bytes if they appear.
    ...
    public float[] parse(@Nullable String str) {
         ...
         * get the size.  It's still prolly worth the initial pass through in order to avoid having to deal with
